...
WSGI_APPLICATION = 'helm_site.wsgi.application'
...
# ---------------------------------------------------------------------------
# Fleet Product Registry — sourced from keel.core.fleet.FLEET (imported above
# as KEEL_FLEET_PRODUCTS). Demo mode rewrites the production hostnames to
# their demo-* equivalents on a per-entry copy so the shared keel module
...
if DEMO_MODE:
    KEEL_FLEET_PRODUCTS = [
        {
            **p,
            'url': p['url'].replace('://', '://demo-'),
            **(
                {'feed_url': p['feed_url'].replace('://', '://demo-')}
                if p.get('feed_url') else {}
            ),
        }
        for p in KEEL_FLEET_PRODUCTS
    ]
...
KEEL_CSP_POLICY = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://fonts.googleapis.com; font-src 'self' data: https://fonts.gstatic.com https://cdn.jsdelivr.net; img-src 'self' data: https:; connect-src 'self'"
